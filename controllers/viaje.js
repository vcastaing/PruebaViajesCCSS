...
const { createViaje, getAllviajes, eliminateViaje, updatingViaje } = require('../models/viaje');
...
const updateViaje = async (req, res) => {
  const idViaje = req.params.idViaje;
  const { nombreSecretaria, nombreChofer } = req.body; 

  if (nombreSecretaria === undefined || nombreChofer === undefined) {
    return res.status(400).json({ error: 'hay campos undefined' });
  }

  console.log(`id controllers updating: ${idViaje}`);
  try {
    const updViaje = await updatingViaje(idViaje, { nombreSecretaria, nombreChofer });
...
module.exports = { postViaje, getAllviajess, deleteViaje, updateViaje };